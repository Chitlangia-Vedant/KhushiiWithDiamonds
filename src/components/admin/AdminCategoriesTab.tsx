      ...
      <div className="flex flex-col p-1 min-w-[320px]">
        ...
        <div className="text-sm text-gray-800 mb-5 pl-[52px] leading-relaxed">
          ...
          <p className="bg-red-50/80 p-2 border border-red-100 rounded text-red-800 text-xs">
            <span className="font-bold text-red-600">Warning:</span> Ensure no items or subcategories are using it first!
          </p>
        </div>
          ...
          <button
            onClick={async () => {
              ...
              try {
                ...
                toast.error('Error deleting category. It might have attached items.', { id: loadingToastId, duration: 5000 });
              }
            }}
    ...
    return (
      <React.Fragment key={category.id}>
        <tr className="hover:bg-gray-50 transition-colors border-b border-gray-100">
          ...
          <td className="px-4 py-3 whitespace-nowrap text-right text-sm font-medium">
            ...
            {hasSubcategories || totalItems > 0 ? (
              <button disabled title="Cannot delete: Move or delete all items and subcategories first." className="p-1 text-gray-300 cursor-not-allowed">
                <Trash2 className="h-4 w-4 md:h-5 md:w-5" />
              </button>
            ) : (
              <button onClick={() => handleDelete(category.id)} title="Delete Category" className="p-1 text-red-600 hover:text-red-900">
                <Trash2 className="h-4 w-4 md:h-5 md:w-5" />
              </button>
            )}
          </td>
        ...
        {hasSubcategories && isExpanded && (
          subcategories.map(sub => <CategoryTableRow key={sub.id} category={sub} level={level + 1} />)
        )}
      </React.Fragment>
    );
  };
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowAddForm(true)} className="bg-yellow-600 text-white px-4 py-2 rounded-lg hover:bg-yellow-700 flex items-center space-x-2 shadow-sm">
      ...
      {showAddForm && (
        <CategoryForm editingCategory={editingCategory} onSuccess={() => { refetchCategories(); resetForm(); }} onCancel={resetForm} />
      )}
    </>
  );
}