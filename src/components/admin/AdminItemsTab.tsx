...
export function AdminItemsTab() {
    ...
    toast((t) => (
      <div className="flex flex-col p-1 min-w-[320px]">
        ...
        <div className="text-sm text-gray-800 mb-5 pl-[52px] leading-relaxed">
          ...
          <p className="bg-red-50/80 p-2 border border-red-100 rounded text-red-800 text-xs"><span className="font-bold text-red-600">Warning:</span> This action cannot be undone and its images will be removed.</p>
        </div>
        <div className="flex justify-end gap-3 mt-1">
          ...
          <button onClick={async () => {
              ...
              try {
                ...
                if (itemToDelete?.image_url && itemToDelete.image_url.length > 0) { try { await deleteDriveImages(itemToDelete.image_url); } catch (e) {} }
              ...
              } catch (error) { toast.error('Error deleting item.', { id: loadingToastId }); }
    ...
    toast((t) => (
      <div className="flex flex-col p-1 min-w-[320px]">
        ...
        <div className="text-sm text-gray-800 mb-5 pl-[52px] leading-relaxed">
          ...
          <p className="bg-red-50/80 p-2 border border-red-100 rounded text-red-800 text-xs"><span className="font-bold text-red-600">Warning:</span> All associated images will also be permanently removed.</p>
        </div>
        <div className="flex justify-end gap-3 mt-1">
          <button onClick={() => toast.dismiss(t.id)} className="px-5 py-2 text-sm font-bold text-gray-700 bg-white hover:bg-gray-50 rounded-lg transition-colors border border-gray-300 shadow-sm">Cancel</button>
          <button onClick={async () => {
            ...
            }} className="px-5 py-2 text-sm font-bold text-white bg-red-600 hover:bg-red-700 rounded-lg transition-colors shadow-sm border border-red-700">Yes, delete all</button>
        </div>
      </div>
    ), { duration: Infinity, style: { maxWidth: '450px', padding: '16px', backgroundColor: '#ffffff', border: '1px solid #fecaca' } });
  };
  ...
  const handleSelectAll = (checked: boolean, pageIds: string[]) => {
    if (checked) {
      setSelectedItemIds(Array.from(new Set([...selectedItemIds, ...pageIds])));
    } else {
  ...
  const filteredItems = useMemo(() => {
    return items.filter(item => {
      ...
      if (activeCategoryName !== 'All') {
        ...
        if (selectedCat) {
          if (!getValidCategoryNames(selectedCat.id, categories).includes(item.category)) return false;
        } else {
